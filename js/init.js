  ...
  "use strict";
	...
	var FrenifyMetaPortal = {
		...
		navSubMenu: function(){
			$('.metaportal_fn_leftnav .nav_holder a').off().on('click',function(){
				...
				if(s.length){
					...
					return false;
				}else if($(e.attr('href')).length){
					$("html, body").animate({ scrollTop: $(e.attr('href')).offset().top }, 1000);
					$('.metaportal_fn_leftnav, .metaportal_fn_leftnav_closer,.metaportal_fn_leftnav .fn__closer').removeClass('active');
					return false;
				}
			});
		},
		...
		headerAnchor: function(){
			$('.header .nav a,.metaportal_fn_mobnav .mob_bot li a').on('click',function(){
				var e = $(this);
