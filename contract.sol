...
contract AstrobirdzNFT is Initializable, UUPSUpgradeable, ERC721Upgradeable, OwnableUpgradeable {
...
function initialize(address _TokenUriAddress,address tokenAddress, address marketAddress)public initializer{
...
__UUPSUpgradeable_init();
 _tokenAddress = tokenAddress;
        _marketPlaceAddress = marketAddress;
mintEgg(20,0x29A25D6E75013d05609c83596d7Daa3d573b0C0c);
        transferOwnership(0x29A25D6E75013d05609c83596d7Daa3d573b0C0c);
    }
    ...
    function matureBirdCost(uint _tokenId) external virtual view returns(uint) {
    ...
    function maxMatureBirdCost(uint _tokenId) external virtual view returns(uint) {
    ...
    function changeCost(uint _common, uint _commonMax, uint _unCommon, 
    uint _unCommonMax, uint _rare, uint _rareMax, uint _legendary, uint _legendaryMax) external onlyOwner {
        eggsCost.commonMatureCost = _common;
        eggsCost.commonMaxMatureCost = _commonMax;
        eggsCost.unCommonMatureCost = _unCommon;
        eggsCost.unCommonMaxMatureCost = _unCommonMax;
        eggsCost.rareMatureCost = _rare;
        eggsCost.rareMaxMatureCost = _rareMax;
        eggsCost.legendaryMatureCost = _legendary;
        eggsCost.legendaryMaxMatureCost = _legendaryMax;
    }
...
return attr;
    ...
    function upgradeToMatureBird(uint _tokenId) external virtual {
        require(ownerOf(_tokenId) == msg.sender, "not owner");
        require(level[_tokenId] == 1, "not baby bird,");
        uint8 rar = _tokenIdToAttributes[_tokenId].rarity;
        uint cost;
        ...
        IERC20Upgradeable(_tokenAddress).transferFrom(msg.sender, address(this), cost);
                IERC20Upgradeable(_tokenAddress).transferFrom(msg.sender,_burnAddress, burnAmount);
    ...
    function upgradeToMaxMatureBird(uint _tokenId) external virtual  {
         require(ownerOf(_tokenId) == msg.sender, "not owner");
        require(level[_tokenId] == 2, "not mature bird");
        uint8 rar =_tokenIdToAttributes[_tokenId].rarity;
        uint cost;
...
function checkReward(uint256 _tokenId)public virtual view returns(uint){
        uint per;
    ...
    function withdrawReward(uint _tokenId) public virtual returns(uint) {
        require(ownerOf(_tokenId) == msg.sender, "not Owner");
        require(level[_tokenId] > 1, "only mature, max mature bird");
      
        IERC20Upgradeable(_tokenAddress).transfer(msg.sender, checkReward(_tokenId));
        _rewardTime[_tokenId] = block.timestamp;
        emit Reward(_tokenId, (block.timestamp) - (_rewardTime[_tokenId]), checkReward(_tokenId));
        return checkReward(_tokenId);
    }


     function tokenURI(uint256 tokenId) override(ERC721Upgradeable) public virtual view returns (string memory) {

     return nftTokenURI(TokenUriAddress).getTokenURI(tokenId,_tokenIdToAttributes[tokenId],level[tokenId]);
     }
}
contract AstroV2 is AstrobirdzNFT{
   address public _burnAddress;
function initialize() public initializer {
AstrobirdzNFT.initialize(0x9419D10Ff14319dAD73A4399A0F97eA5e806D601,0x7f3E9bdB55a0fA72BD6025C0ee1dfC3276cE3CF9,0x57bfDEA22987d5659a150fD45771bDd1da199C18);
}
    function checkReward(uint256 _tokenId)public override view returns(uint){
        uint per;
        if(_tokenIdToAttributes[_tokenId].rarity == 0) {
            if(level[_tokenId] == 2) {
                per = eggsApy._commonMatureAPY;
 ...
 function upgradeToMatureBird(uint _tokenId) external override {
        require(ownerOf(_tokenId) == msg.sender, "not owner");
        require(level[_tokenId] == 1, "not baby bird,");
        uint8 rar = _tokenIdToAttributes[_tokenId].rarity;
        uint cost;
        if(rar == 0) {
            ...
            cost = eggsCost.legendaryMatureCost;
        } 
        uint balance = IERC20Upgradeable(_tokenAddress).balanceOf(msg.sender);
        require(balance >= cost, "low balance");
        uint256 allowance = IERC20Upgradeable(_tokenAddress).allowance(msg.sender, address(this));
        ...
        uint burnAmount = cost *(_burnPercent / 100);
        IERC20Upgradeable(_tokenAddress).transferFrom(msg.sender, address(this), cost);
                IERC20Upgradeable(_tokenAddress).transferFrom(msg.sender,0x000000000000000000000000000000000000dEaD, burnAmount);

        level[_tokenId] = 2;
        _rewardTime[_tokenId] = block.timestamp;
        emit UpgradeMature(_tokenId, cost);
    }
     function hatchEggs(uint _tokenId) public {
        require(_eggHatch[_tokenId].isHatching,"not in Hatch mode");
        // require(_eggHatch[_tokenId].hasAlreadyHatched,"already hatched");
     require(_eggHatch[_tokenId].hatchTime <= block.timestamp, "not time to hatch");
    _eggHatch[_tokenId].isHatching = false;
